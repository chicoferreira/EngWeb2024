...
router.get('/compositores', function (req, res, next) {
    compositorController.list().then(compositores => {
        res.render('compositores', {title: 'Lista de compositores', 'compositores': compositores});
    }).catch(erro => {
...
router.get('/compositores/editar/:id', function (req, res, next) {
    compositorController.findById(req.params.id).then(compositor => {
        res.render('compositorForm', {compositor: compositor, title: 'Editar compositor', id_changeable: false});
    }).catch(erro => {
...
router.get('/compositores/:id', function (req, res, next) {
    compositorController.findById(req.params.id).then(compositor => {
        res.render('compositorIndividual', {title: 'Compositor', 'compositor': compositor});
    }).catch(erro => {
        res.render('error', {error: erro, message: 'Erro ao obter o compositor'});
    });
});
...
router.get('/periodos', function (req, res, next) {
    compositorController.list().then(compositoresResponse => {
...
router.get('/periodos/:periodo', function (req, res, next) {
    ...
    compositorController.findByPeriodo(periodo).then(compositoresResponse => {
        const periodosDict = getPeriodos(compositoresResponse);
